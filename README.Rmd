...
else where cents have to add up. decimal stores values exactly and computes
with them exactly, so what you see is what you have.
...
* **Exact values.** Construction from strings and integers is exact, and
  values round-trip through `as.character()` without loss — nothing changes
  on the way to a CSV file or database column and back.
...
[GitHub](https://github.com/pedrobtz/decimal/issues). When reporting a bug,
a minimal [reprex](https://reprex.tidyverse.org) helps a lot.
